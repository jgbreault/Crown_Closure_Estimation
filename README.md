...
Each script's config section (top of the file / first notebook cell) defines the AOI, paths, and parameters — no CLI arguments needed. `datasets/` is gitignored; everything in it is regenerated by the scripts above, so it doesn't need to be submitted through Git — except the three training patch sets (`datasets/patches/satellite_imagery/`, `datasets/patches/elevation/`, `datasets/patches/crown_closure/`), which are committed.
...
│   ├── crown_closure_pred_segformer_mosaic/  # SegFormer's predictions, whole AOI (committed to Git -- under 100MB in full)
│   └── patches/                            # Per-patch imagery, CSVs, and predictions
│       ├── satellite_imagery/              # Training input, committed to Git
│       ├── elevation/                      # Training input, committed to Git
│       ├── crown_closure/                  # Training label, committed to Git
│       ├── crown_closure_mask/             # Gitignored, regenerated by step 5
│       ├── patch_summaries/                # Gitignored, regenerated by step 6
│       ├── crown_closure_pred_linear/      # Gitignored, regenerated by step 9
│       └── crown_closure_pred_segformer/   # Gitignored, regenerated by step 9
├── models/
...
- [x] Final model-ready dataset, or reproduction instructions — `datasets/patches/{satellite_imagery,elevation,crown_closure}/` committed directly to Git; `datasets/patches/patch_summaries/*.csv` regenerated by steps 1-6 above
